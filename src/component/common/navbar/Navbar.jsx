...
const NavbarComponent = () => {
  ...
  return (
    ...
    <div
    className={`container-fluid fixed-top px-0 ${scrolled ? 'bg-white shadow' : ''}`}
      data-wow-delay="0.1s"
    >
      {/* Top bar */}
        ...
        <Container fluid>
          ...
          <Nav className="ms-auto p-4 p-lg-0">
                        {navItems.map((item, index) =>
                            item.dropdown ? (
                                ...
                                <Nav.Link
                                    key={index}
                                    href={item.href}
                                    className={item.isActive ? 'nav-item nav-link active' : 'nav-item nav-link'}
                                >
                                    {item.name}
                                </Nav.Link>
                            )
            ...
            <div className="d-none d-lg-flex ms-2">
              {/* <Button variant="light" className="btn-sm-square bg-white rounded-circle ms-3">
